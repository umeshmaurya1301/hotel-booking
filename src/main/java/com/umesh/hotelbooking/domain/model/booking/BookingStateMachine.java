...
public final class BookingStateMachine {
     ...
     * {@code CONFIRMED -> CONFIRMED} must be a silent no-op, not a thrown exception. This is
     * idempotency enforced at the state-machine level.
     */
    public boolean canTransition(BookingState from, BookingState to) {
        if (from == to) {
            return true;
        }
     ...
     */
    public void assertCanTransition(BookingState from, BookingState to) {
        if (!canTransition(from, to)) {
            throw new InvalidStateTransitionException(from.name(), to.name());
        }
    }

    public boolean isTerminal(BookingState state) {
